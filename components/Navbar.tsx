    ...
    <nav className={`fixed top-0 left-0 w-full z-50 px-6 py-6 flex justify-between items-center bg-slate-950/80 backdrop-blur-2xl border-b border-white/10 transition-transform duration-500 ease-in-out ${isHidden ? '-translate-y-32' : 'translate-y-0'}`}>
      <div className="flex items-center gap-4 group cursor-pointer" onClick={resetToHome}>
        ...
        <div className="hidden sm:block">
          <h1 className="font-display font-bold tracking-tighter text-2xl leading-none uppercase text-white">Lori <span className="text-red-600 text-glow-red">Battouk</span></h1>
          <p className="text-[9px] tracking-[0.3em] font-bold text-blue-200 uppercase mt-1">Computer Science @ York</p>
        </div>
      </div>
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            {activeSection === item.id && (
              <span className="absolute bottom-0 left-0 w-full h-[2px] bg-red-600 box-shadow-[0_0_10px_#ef4444]"></span>
            )}
          </button>
        ))}
      </div>
